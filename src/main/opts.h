...
inline void UsageDual(){
	...
	std::cout<<"[-g O,P,Z,T,A,B]\n"<<std::endl;
	std::cout<<"    Geometry information: offset,pitch_angle,zshift,thickness,AlignZ,OutBin\n"<<std::endl;
	...
	std::cout<<"[-G GPU number]\n"<<std::endl;
	std::cout<<"    CudaSetDevice\n"<<std::endl;
	std::cout<<"[-h]"<<std::endl;
...
inline void PrintOpts(const options& opt){
	...
	std::cout<<"AlignZ = "<<opt.AlignZ<<std::endl;
	std::cout<<"OutBin = "<<opt.OutBin<<std::endl;
	std::cout<<"GPU = "<<opt.GPU<<std::endl;
...
inline int GetOpts(int argc, char **argv, options* opts_){
	...
	int ch;
	while((ch = getopt_long(argc, argv, "hi:o:n:g:m:a:G", longopts, NULL)) != -1){
		switch (ch){
