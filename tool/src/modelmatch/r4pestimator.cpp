...
// 				std::cout<<matches[j].first<<std::endl;
				find = true;
				break;
			}
		}
...
void Ran4PEstimator::HIterator::ReEstimateTransform(std::vector<std::vector<std::pair<int, int> > >& mmset, std::vector<cv::Mat >& hset)
{	
	mmset.clear();
	hset.clear();
